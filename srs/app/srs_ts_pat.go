...
func (this *SrsTsPayloadPATProgram) Encode(stream *utils.SrsStream) {
	stream.WriteInt16(this.number, binary.LittleEndian)
	var b byte = 0
	this.const1Value = 0x07
	b |= byte(this.const1Value)
	b |= byte(this.pid&0x1f) << 3
	stream.WriteByte(b)
	b = 0
	b |= byte(this.pid>>3) & 0xff
	stream.WriteByte(b)
}
...
func (this *SrsTsPayloadPAT) Encode(stream *utils.SrsStream) {
	...
	s.WriteInt16(this.transportStreamId, binary.LittleEndian)
	this.const1Value0 = 0x03
	var b byte = 0
	b |= byte(this.const1Value0)
	b |= byte((this.versionNumber & 0x1f) << 2)
	b |= byte((this.currentNextIndicator & 0x01) << 7)
	s.WriteByte(b)
	...
	CRC32 := crc32.ChecksumIEEE(s.Data()[1:])
	s.WriteInt32(int32(CRC32), binary.LittleEndian)//4
